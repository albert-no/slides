...
\begin{center}
{\bf Data Privacy: Homeworks \#2 \begin{solution}Solutions\end{solution}}\\ \medskip
...
%     \item
%     \begin{align}
%         \partial_yP_Y(y) =& \int P_X(x) \partial_y P_Z(y-x)\, dx \\
...
%          =& -\frac{y}{\sigma^2} P_Y(y) + P_Y(y) \frac{1}{\sigma^2}\Expec{X|Y}
%     \end{align}
%     Since $\partial_y \log P_Y(y) = \frac{\partial_y P_Y(y)}{P_Y(y)}$, we have
%     \begin{align}
%         \Expec{X|Y} = y + \sigma^2 \partial_y \log P_Y(y).
%     \end{align} \item \begin{align}
%          \partial_y^2 \log P_Y(y) =& \frac{\partial_y^2 P_Y(y)  P_Y(y)- (\partial_y P_Y(y))^2}{P_Y(y)^2}\\
%          =& \frac{\partial_y^2 P_Y(y)}{P_Y(y)} - \frac{(\partial_y P_Y(y))^2}{P_Y(y)^2}
%      \end{align}
%      The second term is simply $(\partial_y \log P_Y (y))^2$.
%      Then,
%      \begin{align}
%          \partial_y^2 P_Y(y) 
%          =& -\partial_y \frac{y}{\sigma^2} P_Y(y) + \partial_y P_Y(y) \frac{1}{\sigma^2}\Expec{X|Y}\\
%          =& - \frac{1}{\sigma^2} P_Y(y) -\frac{y}{\sigma^2}\partial_yP_Y(y) +
%          \partial_y\int P_X(x) \frac{1}{\sqrt{2\pi\sigma^2}} \left(\frac{x}{\sigma^2}\right)\exp\left(-\frac{(x-y)^2}{2\sigma^2}\right)\, dx \\
%          =& - \frac{1}{\sigma^2} P_Y(y) -\frac{y}{\sigma^2}\partial_yP_Y(y) + \int \left(\frac{x(x-y)}{\sigma^4}\right) P_{X|Y}(x|y)P_Y(y)\, dx \\
%          =& - \frac{1}{\sigma^2} P_Y(y) -\frac{y}{\sigma^2}\left(-\frac{y}{\sigma^2} P_Y(y) + P_Y(y) \frac{1}{\sigma^2}\Expec{X|Y}\right)\nonumber\\
%          &+ \frac{1}{\sigma^4}\Expec{X^2|Y}P_Y(y) - \frac{y}{\sigma^4} \Expec{X|Y} P_Y(y)
%      \end{align}
%      Thus, 
%      \begin{align}
%          \frac{\partial_y^2 P_Y(y)}{P_Y(y)} = \frac{1}{\sigma^4}\left(-\sigma^2 +y^2 -2y\Expec{X|Y} + \Expec{X^2|Y}\right)
%      \end{align}
%      On the other hand, 
%      \begin{align}
%          \frac{(\partial_y P_Y(y))^2}{P_Y(y)^2} =& (\partial_y \log P_Y (y))^2\\
%          =& \frac{1}{\sigma^4}(\Expec{X|Y}^2 + y^2 - 2y\Expec{X|Y}).
%      \end{align}
%      Finally, we have
%      \begin{align}
%          \sigma^4\partial_y^2 \log P_Y(y) =& \Expec{X^2|Y} - \Expec{X|Y}^2 -\sigma^2\\
%          =& \Var{X|Y} - \sigma^2
%      \end{align}
%      which implies
%      \begin{align}
%          \Var{X|Y} = \sigma^2 + \sigma^4 \partial_y^2 \log P_Y(y).
%      \end{align}
% \end{enumerate}
% \end{solution}
...
\vskip .2in
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Problem ? %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Let $\sigma_t\geq 0$ be a smooth non-decreasing function for $0\leq t\leq T$. Define
\begin{align}
    \rho(t) = \sqrt{\frac{d}{dt}\sigma_t^2}
\end{align}
for $0\leq t\leq T$.
Consider the SDE
\begin{align}
    dX_t = \rho(t) dW_t
\end{align}
with initial condition $X_0\sim p_0$ where $W_t$ is standard Brownian motion.
Show that $X_t|X_0\sim \cN(X_0, \sigma_t^2)$ by verifying that
\begin{align}
    p_t(x) = \int p_{t|0} (x|y)p_0(y)\, dy = \int \frac{1}{\sqrt{2\pi\sigma_t^2}} \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right) p_0(y)\, dy
\end{align}
...
\begin{align}
    \partial_t p_t = \frac{\rho(t)^2}{2} \partial_x^2 p_t.
\end{align}
The left hand side is
\begin{align}
    \partial_t p_t 
    =& \int \partial_t\left( \frac{1}{\sqrt{2\pi\sigma_t^2}} \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right)\right) p_0(y)\, dy\\
    =& \int \partial_t \frac{1}{\sqrt{2\pi\sigma_t^2}} \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right) p_0(y)\, dy\nonumber\\
    &+ \int \frac{1}{\sqrt{2\pi\sigma_t^2}} \partial_t \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right) p_0(y)\, dy\\
    =& \int  -\frac{\partial_t \sigma_t^2}{2\sqrt{2\pi}(\sigma_t^2)^{3/2}} \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right) p_0(y)\, dy\nonumber\\
    &+ \int \frac{1}{\sqrt{2\pi\sigma_t^2}}(\partial_t\sigma_t^2)  \left(\frac{(x-y)^2}{2\sigma_t^4}\right)\exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right) p_0(y)\, dy\\
    =&\frac{\rho(t)^2}{2} \int  -\frac{1}{\sigma_t^2}\frac{1}{\sqrt{2\pi\sigma_t^2}} \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right) p_0(y)\, dy\nonumber\\
    &+ \frac{\rho(t)^2}{2}\int \frac{1}{\sqrt{2\pi\sigma_t^2}}  \left(\frac{(x-y)^2}{\sigma_t^4}\right)\exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right) p_0(y)\, dy
\end{align}
where $\partial_t \sigma_t^2 = \rho(t)^2$.

On the other hand
\begin{align}
    \partial_x^2 p_t 
    =&  \int \partial_x\left( \frac{1}{\sqrt{2\pi\sigma_t^2}}\left(-\frac{x-y}{\sigma_t^2}\right) \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right)\right) p_0(y)\, dy\\
    =&  \int \left( \frac{1}{\sqrt{2\pi\sigma_t^2}}\left(-\frac{1}{\sigma_t^2}\right) \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right)\right) p_0(y)\, dy\nonumber\\
    &+  \int \left( \frac{1}{\sqrt{2\pi\sigma_t^2}}\left(\frac{(x-y)^2}{\sigma_t^4}\right) \exp\left(-\frac{(x-y)^2}{2\sigma_t^2}\right)\right) p_0(y)\, dy.
\end{align}
...
Consider the ODE
\begin{align}
dX_t = \left(f(X_t, t) - \frac{g^2(t)}{2} \partial_{X_t} \log p_t(X_t) \right) dt
\end{align}
with terminal condition $X_T \sim p_T$. 
Show that $\{p_t\}_{t=0}^T$ satisfies the Fokker–Planck equation
\begin{align}
    \partial_t p_t = -\partial_x (fp_t) + \frac{g^2}{2}\partial_x^2 p_t.
\end{align}
Hint: See the proof of Anderson's theorem and its derivation of the Fokker-Planck equation with SDE.

%%%%%%%%%%%%%%%%%%%%%%%% Problem 5 Solution %%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{solution}
{\bf Solution:}\\
Let the reverse SDE be
\begin{align}
    d\bar{X}_t = (f(\bar{X}_t, t) - \frac{g^2(t)}{2} \nabla_x\log p_t(\bar{X}_t))dt
\end{align}
with $\bar{X}_T\sim p_T$.
Alternatively, define $\{Y_t\}_{t=0}^T$ where $Y_t= \bar{X}_{T-t}$via
\begin{align}
    dY_t =  - (f(Y_t, T-t) - \frac{g^2(T-t)}{2}\nabla_x \log p_{T-t}(Y_t))dt
\end{align}
with $Y_0\sim p_T$.
...
\begin{align}
    \partial_t q_t(y) = \partial_y \left((f(y, T-t) - \frac{g^2(T-t)}{2}\partial_y \log p_{T-t}(y))q_t(y)\right).
\end{align}
...
\begin{align}
    \partial_t \bar{p}_t(x) = -\partial_x \left((f(x, t) - \frac{g^2(t)}{2}\partial_x \log p_{t}(x))\bar{p}_t(x)\right)\label{eq:reverse SDE}.
\end{align}

The final step of the proof is proving that $p_t$ solves the above reverse FP equation.
\begin{align}
    &-\partial_x \left((f(x, t) - \frac{g^2(t)}{2}\partial_x \log p_{t}(x))p_t(x)\right)\\
...
\vskip .2in
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Problem 7 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item \textbf{Star shaped diffusion.}
Let $q$ be the forward process of star-diffusion model, which is given by
\begin{align*}
