...
In scalar case, we have following property to generate samples from uniform distribution.
\begin{exercise}
...
\begin{exercise}
    How can we generate a uniform random variable?
\end{exercise}
...
\begin{remark}
    ...
    This fact is useful in training $f_\theta$.
\end{remark}
...
Now, consider the reverse sampling (or reverse process).
For $1\leq n\leq N$, we need a reverse conditional distribution.
For simplicity, let $v = \tx^{(n)}$ and $u = \tx^{(n-1)}$
\begin{align}
     \Puv{u|v}
     =& \frac{\Pvu{v|u} \Pu{u}}{\Pv{v}}\\
     =& \frac{\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2\sigma^2}(u-v)^2\right) \Pu{u}}{\Pv{v}}\\
     =& \frac{\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2\sigma^2}(u-v)^2\right) \Pu{u}}
     {\int_{u}\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2\sigma^2}(u-v)^2\right) \Pu{u} \,du}\\
     =& \frac{\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2\sigma^2}(u-v)^2\right) \Pu{u}}
     {\fE_{T\sim \cN(v, \sigma^2)} [\Pu{T}] }
\end{align}
where the last expectation is with respect to density function $T\sim\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2\sigma^2}(t-v)^2\right)$.
Suppose $\sigma \ll 1$ is small then we have $t\approx v$, and
\begin{align}
    \Pu{t} =& \Pu{v} +  \left.\frac{\partial\Pu{t}}{\partial t}\right|_{t=v} (t-v) + O((t-v)^2)
\end{align}
Thus, $\fE_{T\sim \cN(\bv, \sigma^2 )} [\Pu{T}] = \Pu{v} + 0 + O(\sigma^2)$, and
\begin{align}
     \Puv{u|v}
     \approx& \frac{\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2\sigma^2}(u-v)^2\right) } {\Pu{v}}
     \left( \Pu{v} + \frac{\partial\Pu{v}}{\partial v}(u-v)\right)\\
     =& \frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2\sigma^2}(u-v)^2\right) 
     \left( 1 + \left(\frac{\partial}{\partial v}\log \Pu{v}\right) (u-v)\right)\\
     \approx& \frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2\sigma^2}\left(u-v - \sigma^2\frac{\partial}{\partial v}\log \Pu{v}\right)^2\right).
\end{align}
\begin{exercise}
...
Finally, we have
\begin{align}
    \left(\tX^{(n-1)} | \tX^{(n)}=\tx^{(n)} \right)\sim \cN\left(\tx^{(n)} 
    + \sigma^2 \frac{\partial}{\partial x} \log P_{\tX^{(n)}}(\tx^{(n)}), \sigma^2 \right).
\end{align}
This implies that ``if $\sigma$ is small, we can approximate the reverse conditional distribution with Gaussian as well.''
Then, we generate samples via
    \begin{align}
        \tx^{(n-1)} = \tx^{(n)} + \sigma^2 \frac{\partial}{\partial x} \log P_{\tX^{(n)}} (\tx^{(n)}) + \tZ^{(n)}
    \end{align}
where $\tZ^{(n)}\sim\cN(0, \sigma^2)$ is an independent noise

\begin{exercise}
    Is there a difference between subtracting noise versus adding noise?
    What if we generate samples via subtraction? I.e.,
    \begin{align}
        \tX^{(n-1)} = \tX^{(n)} - \tZ^{(n)}
    \end{align}
\end{exercise}
...
Consider the single sample $x_0=x$ and its corrupted version $x_N$.
If we sample from $X_N = x_N$ and generate $X_0$ through optimum denoising, we may not (and will not) get sample that is similar to original $x$.
...
\begin{remark}
    ...
    if we have full information of ``score function'' $\frac{\partial}{\partial x} \log P_{\tX^{(n)}} (\tx^{(n)})$ for all $n$ and all $\tx^{(n)}$.
...
Now suppose we do not have any information about probability density function or score function $s(x; n) = \frac{\partial}{\partial x} \log P_{\tX^{(n)}} (x)$.
Note that your score function is also a function of $n$ since the density function $P_{\tX^{(n)}}$ depends on $n$.
We can train neural network $f_{\theta}(x; n)$ to approximate the score function.
Note that we can have pairs of $(x^{(0)}, x^{(n)})$ for $1\leq n\leq N$ where $x^{(0)}$ can be obtained from given samples.
Then, train network to minimize the difference $(f_{\theta}(x^{(n)};n) - x^{(0)})$, then we will eventually obtain
\begin{align}
    f_{\theta}(x;n) \approx \fE_{X^{(0)}|X^{(n)}} [X^{(0)}| X^{(n)}=x].
\end{align}
This training process does not require any knowledge of distribution or score function.
...
\end{align}
Thus, we can approximate score function with
\begin{align}
    \hat{s}(x;n) = \frac{f_{\theta}(x; n) - x}{n}
\end{align}
...
This is called Denoising Diffusion Probabilistic Models (DDPM)~\cite{ho2020denoising} (note that it has $>7000$ citations already).
...
where
\begin{align}
    \mu(X^{(n)}, n) = \frac{1}{\sqrt{1-\beta_n}} \left(X^{(n)} + \beta_n \frac{\partial}{\partial x} \log p_n(X^{(n)})\right).
\end{align}
Note tha the $\mu(X^{(n)}, n)$ is from Tweedie's formula, and we can similarly show that the variance of $X^{(n-1)}|X^{(n)}$ is also $\beta_n$ with a negligible high order terms.
...
The goal of generative model is minimizing negative log-likelihood (why?).
\begin{align}
    -\log p_{\theta}(X^{(0)}) =& -\log \int p_{\theta}(X^{(0:N)})\, dX^{(1:N)}\\
    =& -\log \left[\int \frac{p_{\theta}(X^{(0:N)})}{q(X^{(1:N)}|X^{(0)})} q(X^{(1:N)}|X^{(0)})\, dX^{(1:N)}\right]\\
    =& -\log \fE_q\left[\left.\frac{p_{\theta}(X^{(0:N)})}{q(X^{(1:N)}|X^{(0)})} \right|X_0\right]\\
    \leq& \fE_q\left[-\log \left.\frac{p_{\theta}(X^{(0:N)})}{q(X^{(1:N)}|X^{(0)})} \right|X_0\right].
\end{align}
The idea is that instead of minimizing neagtive log-likelihood directly,
...
\begin{align}
    ...
    &= \fE_{X^{(0)}\sim q}\left[D_{KL}(q(X^{(N)}|X^{(0)}) \| p(X^{(N)}))\right].
\end{align}
...
\begin{align}
    ...
    =& \sum_{n=2}^N \fE_q \left[ D (q(X^{(n-1)}|X^{(n)}, X^{(0)}) \| p_{\theta}(X^{(n-1)}|X^{(n)})\right]\\
    =& \sum_{n=2}^N \fE_q \left[ D (q(X^{(n-1)}|X^{(n)}, X^{(0)}) \| p_{\theta}(X^{(n-1)}|X^{(n)})\right]\\
...
\begin{exercise}
    ...
    \begin{align}
        \frac{\partial}{\partial_{X^{(n)}}} \log q (X^{(n)}|X^{(0)})
    \end{align}
\end{exercise}
...
\begin{exercise}
    ...
    \begin{align}
        ...
        = \frac{1}{2\sigma_1^2} \|\mu_1-\mu_0\|^2 + \frac{(\sigma_0^2/\sigma_1^2 -1)d}{2} + d\log \frac{\sigma_0}{\sigma_1}.
    \end{align}
...
\end{align}
where $H_{p_t}$ is Hessian of $p_t(\bx)$.
...
\begin{align}
    c_{\phi}(\bX^{(n)}, Y, n) \approx P_n(Y|\bX^{(n)}).
\end{align}
...
%%% Universal Guidance
\subsection{Universal Guidance~\cite{bansal2024universal}}
\noa{Need to sync the notation}

Let $\bX^{(t)} = \sqrt{\bar{\alpha}_t} \bX^{(0)} + \sqrt{1-\bar{\alpha}_t} \epsilon$
where $\epsilon\sim~\cN(0, I)$.
We have seen that the diffusion model learns the noise, i.e.,
\begin{align}
    \epsilon_{\theta}(\bX^{(t)}, t) \approx \epsilon = \frac{\bX^{(t)} - \sqrt{\bar{\alpha}_t} \bX^{(0)}}{\sqrt{1-\bar{\alpha}_t}}.
\end{align}

DDIM~\cite{song2020denoising} first predict a clean data
\begin{align}
    \hat{\bX}^{(0)} = \frac{\bX^{(t)} - \sqrt{1-\bar{\alpha}_t} \epsilon_\theta(\bX^{(t)}, t)}{\sqrt{\bar{\alpha}_t}}\label{eq:predict clean}
\end{align}
then sample $\bX^{(n-1)}$ from $q(\bX^{(n-1)}|\bX^{(n)}, \hat{\bX}^{(0)})$.

Universal guidance generalizes the classifier guidance.
Recall that the classifier guidance employs \noa{TBD: use $\hat{\epsilon}$ everywhere}
\begin{align}
    \hat{\epsilon}_{\theta} (\bX^{(n)}, n) = \epsilon(\bX^{(n)}, n) - \sqrt{1-\bar{\alpha}_n}\nabla_{\bX^{(n)}} \log p(y|\bX^{(n)}).
\end{align}
A name ``universal'' suggests that we want to guide a sample with any function $f$ and the loss function $\ell$.
Suppose that the classifier is given only at the clean image, then we may want to apply 
\begin{align}
    \hat{\epsilon}_{\theta} (\bX^{(n)}, n) = \epsilon(\bX^{(n)}, n) + \sqrt{1-\bar{\alpha}_n}\nabla_{\bX^{(n)}} \ell_{ce}(y, f_{cl}(\bX^{(n)}))
\end{align}
where $\ell_{ce}$ is cross entropy loss and $f_{cl}$ is a classifier (classifies clean image).
However, this may not work well since $\bX^{(n)}$ is noise-corrupted version of image while $f_{cl}$ is trained only on clean images.
To overcome the issue, the authors first predict the clean image, and then applied the same formula, i.e.,
\begin{align}
    \hat{\epsilon}_{\theta} (\bX^{(n)}, n) = \epsilon(\bX^{(n)}, n) + \omega(t) \sqrt{1-\bar{\alpha}_n}\nabla_{\bX^{(n)}} \ell(y, f(\hat{\bX}^{(0)}))
\end{align}
where we wrote the generic formula for any guidance function $f$ and loss function $\ell$.
Also, $\omega(t)$ is newly introduced to strength the guidance power (similar to the weight in classifier and classifier-free guidance).
We can predict a clean image using \eqref{eq:predict clean}.
This process is called ``forward universal guidance'' (note that it is part of reverse process that generates sample, but it is forward in terms of guidance).

However, the result is unsatisfactory mainly due to prediction error.
Thus, the we introduce backward guidance (backward direction while generating sample).
Once predict the clean image $\bX^{(0)}$, the backward process finds the best match in clean image space, i.e.,
\begin{align}
    \Delta \bX^{(0)} = \argmin_{\Delta} \ell(y, f(\hat{\bX}^{(0)}+\Delta)). \label{eq:compute Delta bX0}
\end{align}
The above minimization can be solved with a few gradient descent step.
Then, we presume the noisy backward sample $\bX^{(n)}$ is corrupted version of $\bX^{(0)}+\Delta \bX^{(0)}$, i.e.,
\begin{align}
    \bX^{(n)} = \sqrt{\bar{\alpha}_n}(\bX^{(0)} + \Delta\bX^{(0)})+\sqrt{1-\bar{\alpha}_n }\tilde{\epsilon}.
\end{align}
Thus, we now have modified noise from $\epsilon$ to $\tilde{\epsilon}$, where
\begin{align}
    \tilde{\epsilon} = \epsilon - \sqrt{\bar{\alpha}_n / (1-\bar{\alpha}_n)} \Delta\bX^{(0)}
\end{align}
The backward step corrects the noise estimate by
\begin{align}
    \hat{\epsilon}_\theta \leftarrow \hat{\epsilon}_{\theta} - \sqrt{\bar{\alpha}_n / (1-\bar{\alpha}_n)} \Delta\bX^{(0)}\label{eq:update score in backward}
\end{align}

Indeed, the forward and backward guidance are also not enough, and the sampling process requires a self-recurrence
which injects the noise again (apply forward corruption process).
Similar to predictor-corrector~\cite[Seciton 4.2]{song2021scorebased} or Langevin MCMC~\cite{welling2011bayesian}, it somehow matches the distribution.
\begin{align}
    \bX^{(n)} = \sqrt{\bar{\alpha}_n / \bar{\alpha}_{n-1}} \bX^{(n-1)} + \sqrt{1-\bar{\alpha}_n/\bar{\alpha}_{n-1}} \epsilon'\label{eq:self recurrence}
\end{align}
where $\epsilon`\sim\cN(0, I)$.

At each step, the universal guidance iteratively applying forward and backward guidance to correct the error.
In summary,
\begin{enumerate}
    \item Sample $\bX^{(N)}$
    \item for $n=N, N-1, \dots, 1$
    \begin{enumerate}
        ...
        \begin{enumerate}
            \item {\bf Forward}: estimate $\hat{\bX}^{(0)}$ from $\bX^{(n)}$, then compute $\hat{\epsilon}_{\theta}$ 
            \item {\bf Backward}: compute $\Delta \bX^{(0)}$ using \eqref{eq:compute Delta bX0},
                then  update $\hat{\epsilon}_{\theta}$ using \eqref{eq:update score in backward}
            \item {\bf Self Recurrence}: Sample $\bX^{(n-1)}$ using updated $\hat{\epsilon}_{\theta}$,
                then apply self recurrence~\eqref{eq:self recurrence} to reobtain $\bX^{(n)}$
        \end{enumerate}
