...
\emph{Hint.} Write $I = h(\bX) - h(\bX\mid\bXh)$; use independence on the first term, the chain rule on the second, and ``conditioning reduces entropy.''
\item {\bf Converse.}~{(\it 10 points)} Show that
\[
    R(D) \;\ge\; \min_{D_1+D_2\le D}\big(R_1(D_1)+R_2(D_2)\big).
\]

\emph{Hint.} Take any test channel feasible for $R(D)$; set $D_i=\E[(X_i-\hat X_i)^2]$ so $D_1+D_2\le D$. Apply part (a), then bound each $I(X_i;\hat X_i)$ below using the definition of the single-letter $R_i$.
...
\emph{Hint.} Fix any split $(D_1,D_2)$ with $D_1+D_2\le D$ and build a \emph{product} test channel $p(\hat{\mathbf x}\mid\mathbf x)=\prod_i p_i^\star(\hat x_i\mid x_i)$ from the per-component optimizers; compute its mutual information and its distortion.
\end{enumerate}
...
\begin{enumerate}
\item {\bf Foundations.}~{(\it 10 points)} Writing the likelihood ratio as $p/q$, show $\chi^2(P\|Q)=\mathrm{Var}_Q\!\big(\tfrac{p}{q}\big)$, and that $D(P\|Q)\le\chi^2(P\|Q)$.

\emph{Hint.} $\log t\le t-1$.
\item {\bf Hammersley--Chapman--Robbins.}~{(\it 15 points)} For any statistic $T$ with $\E_P[T]=a$, $\E_Q[T]=b$, prove
\[
    \mathrm{Var}_Q(T) \;\ge\; \frac{(a-b)^2}{\chi^2(P\|Q)}.
\]
Then, for a parametric family $\{P_\theta\}$ with an unbiased estimator of $\theta$, take $Q=P_\theta$, $P=P_{\theta+h}$ and let $h\to0$ to recover the Cram\'er--Rao bound $\mathrm{Var}_\theta(T)\ge 1/I(\theta)$.

\emph{Hint.} Express $a-b$ as a covariance under $Q$ and apply Cauchy--Schwarz.
\item {\bf A $\chi^2$ upper bound on the evidence (CUBO).}~{(\it 10 points)} For a latent-variable model $p(x)=\int p(x,z)\,dz$, a proposal $q(z\mid x)$, and weight $w=\frac{p(x,z)}{q(z\mid x)}$, define $\mathrm{CUBO}_2=\frac12\log\E_{q(z\mid x)}[w^2]$. Show $\log p(x)\le\mathrm{CUBO}_2$, that the gap is exactly $\frac12\log\big(1+\chi^2(p(z\mid x)\|q(z\mid x))\big)$, and hence $\mathrm{ELBO}\le\log p(x)\le\mathrm{CUBO}_2$.

\emph{Hint.} $p(x)=\E_q[w]$ and $(\E_q[w])^2\le\E_q[w^2]$; then write $w=p(x)\,\frac{p(z\mid x)}{q(z\mid x)}$.
\end{enumerate}
...
\begin{enumerate}
\item {\bf (Foundations.)} Let $W=p/q$ be the likelihood ratio. Its first two $Q$-moments are
\[
...
\emph{Cram\'er--Rao limit.} Assume $\{P_\theta\}$ is smooth in $\theta$ with finite Fisher information. Put $Q=P_\theta$, $P=P_{\theta+h}$. Unbiasedness means $\E_{\theta'}[T]=\theta'$, so $a=\theta+h$, $b=\theta$, and $a-b=h$; thus $\mathrm{Var}_\theta(T)\ge h^2/\chi^2(P_{\theta+h}\|P_\theta)$. Taylor-expanding the density, $p_{\theta+h}=p_\theta+h\,\partial_\theta p_\theta+O(h^2)$, so $(p_{\theta+h}-p_\theta)^2=h^2(\partial_\theta p_\theta)^2+O(h^3)$ and
\[
...
\item {\bf (CUBO.)} Assume $q(z\mid x)>0$ wherever $p(z\mid x)>0$, and $\E_q[w^2]<\infty$ so that $\mathrm{CUBO}_2$ is finite. First, $p(x)$ is the $q$-average of the weight:
\[
