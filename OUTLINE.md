...
- **`courses/deepmath/`** — Deep Learning Math course (**junior undergrad AI majors**). Mathematical foundations: probability/information theory/estimation (9 decks, from `probability25.tex` + 2 added topics) and linear algebra/optimization (4 decks, from `optimization25.tex`). Full theorem–proof–implication arcs at slow pace; AI-application motivation per deck (CE loss, diffusion, Netflix completion, SGD). The rigorous-undergrad counterpart to the grad treatments in `infotheory/` and `privacy/`. See `courses/deepmath/OUTLINE.md`.
