...
\[
    R(D) = \min_{p(\hat{\mathbf x}| \mathbf x):\,\E\|\bX-\bXh\|^2\le D} I(\bX;\bXh),
\]
and write $R_i$ for the single-component function $R_i(D_i)=\big[\tfrac12\log\tfrac{\sigma_i^2}{D_i}\big]^{+}$, i.e.~$\tfrac12\log(\sigma_i^2/D_i)$ for $0<D_i\le\sigma_i^2$, $0$ for $D_i\ge\sigma_i^2$, and $R_i(0)=+\infty$. Rates are in bits, $\log=\log_2$. The goal is to prove
\[
    R(D) = \min_{\substack{D_1,D_2\ge 0\\ D_1+D_2\le D}} \big(R_1(D_1)+R_2(D_2)\big).
\]
\begin{enumerate}
\item {\bf Decoupling lemma.}~{(\it 10 points)} For \emph{any} joint distribution of $(\bX,\bXh)$, show
\[
    I(\bX;\bXh) \;\ge\; I(X_1;\hat X_1) + I(X_2;\hat X_2).
\]

\emph{Hint.} Write $I = h(\bX) - h(\bX|\bXh)$; use independence on the first term, the chain rule on the second, and ``conditioning reduces entropy.''
\item {\bf Converse.}~{(\it 10 points)} Show that
\[
    R(D) \;\ge\; \min_{D_1+D_2\le D}\big(R_1(D_1)+R_2(D_2)\big).
\]

\emph{Hint.} Take any test channel $p(\hat{\mathbf x}|\mathbf x)$ that satisfies $\E\|\bX-\bXh\|^2\le D$; set $D_i=\E[(X_i-\hat X_i)^2]$ so $D_1+D_2\le D$. Apply part (a), then bound each $I(X_i;\hat X_i)$ below using the definition of the single-letter $R_i$.
\item {\bf Achievability.}~{(\it 15 points)} Show the matching upper bound
\[
    R(D) \;\le\; \min_{D_1+D_2\le D}\big(R_1(D_1)+R_2(D_2)\big),
\]
so that equality holds.

\emph{Hint.} Fix any split $(D_1,D_2)$ with $D_1+D_2\le D$ and build a \emph{product} test channel $p(\hat{\mathbf x}|\mathbf x)=\prod_i p_i^\star(\hat x_i| x_i)$ from the per-component optimizers; compute its mutual information and its distortion.
\end{enumerate}
...
\begin{enumerate}
\item {\bf (Decoupling lemma.)} Write the mutual information in differential-entropy form, $I(\bX;\bXh)=h(\bX)-h(\bX|\bXh)$, and treat the two terms separately.
...
\[
    h(\bX|\bXh) = h(X_1|\bXh) + h(X_2| X_1,\bXh).
\]
...
\[
    h(X_1|\bXh) \le h(X_1|\hat X_1),
    \qquad
    h(X_2| X_1,\bXh) \le h(X_2|\hat X_2),
\]
because $\bXh=(\hat X_1,\hat X_2)$ contains $\hat X_1$, and $(X_1,\bXh)$ contains $\hat X_2$. Adding the two, $h(\bX|\bXh)\le h(X_1|\hat X_1)+h(X_2|\hat X_2)$. Combining the two terms,
\[
    I(\bX;\bXh) = h(\bX)-h(\bX|\bXh)
    \ge \sum_{i=1}^2\big(h(X_i)-h(X_i|\hat X_i)\big) = \sum_{i=1}^2 I(X_i;\hat X_i). \qquad\blacksquare
\]
\item {\bf (Converse.)} Let $p(\hat{\mathbf x}|\mathbf x)$ be \emph{any} test channel feasible for $R(D)$, and define $D_i:=\E[(X_i-\hat X_i)^2]$ for the distortion it induces on each component. Summing, $D_1+D_2=\E\|\bX-\bXh\|^2\le D$, so $(D_1,D_2)$ is a feasible split. The channel's marginal $X_i\to\hat X_i$ is itself a single-letter test channel achieving distortion $D_i$, and the scalar rate--distortion function $R_i(D_i)$ is by definition the \emph{smallest} mutual information over all such channels; therefore $I(X_i;\hat X_i)\ge R_i(D_i)$. Chaining this with part (a),
\[
    I(\bX;\bXh)
...
\]
\item {\bf (Achievability.)} Fix any split $(D_1,D_2)$ with $D_1+D_2\le D$; we may take $0<D_i\le\sigma_i^2$, since raising $D_i$ above $\sigma_i^2$ leaves $R_i=0$ while only loosening the distortion budget (and $D_i\to0$ is the lossless limit, $R_i\to\infty$), so it never helps the minimum. For each component let $p_i^\star(\hat x_i| x_i)$ be the optimal scalar Gaussian test channel attaining $I(X_i;\hat X_i)=R_i(D_i)$ at distortion exactly $D_i$, and form the \emph{product} channel
\[
    p(\hat{\mathbf x}|\mathbf x)=p_1^\star(\hat x_1| x_1)\,p_2^\star(\hat x_2| x_2).
\]
...
\begin{enumerate}
\item {\bf Foundations.}~{(\it 10 points)} Define the likelihood ratio $W=\frac{p(X)}{q(X)}$. Show $\chi^2(P\|Q)=\mathrm{Var}_Q(W):=\E_Q\big[(W-\E_Q[W])^2\big]$, and that $D(P\|Q)\le\chi^2(P\|Q)$.

\emph{Hint.} $\log t\le t-1$.
\item {\bf Hammersley--Chapman--Robbins.}~{(\it 15 points)} For any statistic $T$, prove
\[
    \mathrm{Var}_Q(T) \;\ge\; \frac{\big(\E_P[T]-\E_Q[T]\big)^2}{\chi^2(P\|Q)}.
\]

\emph{Hint.} Write $\E_P[T]-\E_Q[T]=\E_Q[T(W-1)]$, then apply the Cauchy--Schwarz inequality $\E[XY]^2\le\E[X^2]\,\E[Y^2]$.
\item {\bf A $\chi^2$ upper bound on the evidence (CUBO).}~{(\it 10 points)} For a latent-variable model $p(x)=\int p(x,z)\,dz$, a proposal $q(z|x)$, and weight $W=\frac{p(x,Z)}{q(Z|x)}$ with $Z\sim q(\cdot|x)$, define $\mathrm{CUBO}_2=\frac12\log\E_Q[W^2]$ (where $\E_Q$ is the expectation over $Z\sim q(\cdot|x)$). Show $\log p(x)\le\mathrm{CUBO}_2$, that the gap is exactly $\frac12\log\big(1+\chi^2(p(z|x)\|q(z|x))\big)$, and hence $\mathrm{ELBO}\le\log p(x)\le\mathrm{CUBO}_2$.

\emph{Hint.} $p(x)=\E_Q[W]$ and $(\E_Q[W])^2\le\E_Q[W^2]$; then write $W=p(x)\,\frac{p(Z|x)}{q(Z|x)}$.
\end{enumerate}
...
\begin{enumerate}
\item {\bf (Foundations.)} Let $W=\frac{p(X)}{q(X)}$ be the likelihood ratio. Its first two $Q$-moments are
\[
    \E_Q[W]=\int q\,\frac pq = \int p = 1,
    \qquad
    \E_Q[W^2]=\int q\,\Big(\frac pq\Big)^2=\int\frac{p^2}{q}.
\]
Hence $\mathrm{Var}_Q(W)=\E_Q\big[(W-\E_Q[W])^2\big]=\E_Q[W^2]-(\E_Q[W])^2=\int\frac{p^2}{q}-1=\chi^2(P\|Q)$, the first claim. For the comparison with KL, apply $\log t\le t-1$ with $t=p/q$ pointwise:
\[
    D(P\|Q)=\int p\log\frac pq \;\le\; \int p\Big(\frac pq-1\Big)=\int\frac{p^2}{q}-\int p=\int\frac{p^2}{q}-1=\chi^2(P\|Q).
\]
\item {\bf (Hammersley--Chapman--Robbins.)} Assume $P\ne Q$ and $\chi^2(P\|Q)<\infty$, so the bound is non-vacuous. Write the mean gap as a $Q$-expectation against $W$. Since $\E_Q[W]=1$,
\[
\begin{aligned}
    \E_P[T]-\E_Q[T]&=\int T\,p-\int T\,q=\int T(p-q)\\
       &=\int q\,T\Big(\frac pq-1\Big)=\E_Q\big[T(W-1)\big].
\end{aligned}
\]
Because $\E_Q[W-1]=0$, we may center $T$ at no cost: $\E_Q[T(W-1)]=\E_Q\big[(T-\E_Q[T])(W-1)\big]$. Apply the Cauchy--Schwarz inequality $\E[XY]^2\le\E[X^2]\,\E[Y^2]$ with $X=T-\E_Q[T]$ and $Y=W-1$:
\[
    \big(\E_P[T]-\E_Q[T]\big)^2\le\E_Q\big[(T-\E_Q[T])^2\big]\,\E_Q\big[(W-1)^2\big]=\mathrm{Var}_Q(T)\,\chi^2(P\|Q),
\]
where $\E_Q\big[(W-1)^2\big]=\mathrm{Var}_Q(W)=\chi^2(P\|Q)$ by part (a). Rearranging gives $\mathrm{Var}_Q(T)\ge\big(\E_P[T]-\E_Q[T]\big)^2/\chi^2(P\|Q)$. $\qquad\blacksquare$
\item {\bf (CUBO.)} Assume $q(z|x)>0$ wherever $p(z|x)>0$, and $\E_Q[W^2]<\infty$ so that $\mathrm{CUBO}_2$ is finite. First, $p(x)$ is the $q$-average of the weight:
\[
    p(x)=\int p(x,z)\,dz=\int q(z|x)\,\frac{p(x,z)}{q(z|x)}\,dz=\E_Q[W].
\]
...
\emph{Exact gap.} Factor the weight through the true posterior, $W=\frac{p(x,Z)}{q(Z|x)}=p(x)\,\frac{p(Z|x)}{q(Z|x)}$ (using $p(x,z)=p(x)\,p(z|x)$). Then
\[
\begin{aligned}
    \E_Q[W^2]&=p(x)^2\,\E_Q\!\Big[\Big(\frac{p(Z|x)}{q(Z|x)}\Big)^2\Big]
    =p(x)^2\int q(z|x)\,\frac{p(z|x)^2}{q(z|x)^2}\,dz\\
    &=p(x)^2\int\frac{p(z|x)^2}{q(z|x)}\,dz.
\end{aligned}
\]
By the foundation identity $\int\frac{p^2}{q}=1+\chi^2(p\|q)$ from part (a), the last integral is $1+\chi^2(p(z|x)\|q(z|x))$, so
\[
    \mathrm{CUBO}_2=\tfrac12\log\E_Q[W^2]=\log p(x)+\tfrac12\log\!\big(1+\chi^2(p(z|x)\|q(z|x))\big).
\]
The gap $\tfrac12\log(1+\chi^2)\ge0$, and vanishes iff $\chi^2=0$, i.e.~$q(z|x)=p(z|x)$. Finally recall the ELBO is $\E_Q[\log W]\le\log\E_Q[W]=\log p(x)$ (Jensen, concavity of $\log$). Combining the two,
\[
    \mathrm{ELBO}\le\log p(x)\le\mathrm{CUBO}_2,
\]
bracketing the intractable log-evidence between two computable bounds. The ELBO gap is the \emph{reverse} KL $D(q(z|x)\|p(z|x))$ (mode-seeking), while the CUBO gap is driven by $\chi^2(p(z|x)\|q(z|x))$ (mass-covering). $\qquad\blacksquare$
\end{enumerate}
\end{solution}
\end{enumerate}
