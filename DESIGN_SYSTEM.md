...
**Division of labor.** This file is the canonical *how to author* reference; `GOTCHAS.md` is the *symptom → cause → fix* debugging reference. On overlap, this file wins. Read this when writing; go to GOTCHAS when something looks wrong.
...
Important content (audience must read it during the talk) → body size. Non-important content → companion `<deck>-note.html`, *not* shrunk to fit.

- The canonical scale is `reference/deck.css`. Don't redefine `p`, `li`, `h1`, `h2`, `h3`, `.subtitle`, `.cite`, `.small`, `.tiny`, or `.math-block` `font-size` in a deck `<style>`. No inline `style="font-size:…"` on prose.
...
- Exactly two sub-body slots: `<div class="cite">` citations, and `.small` inside a diagram (only when the label is a non-crucial sub-label *and* compression breaks the layout).
- **De-emphasis is by color, not size.** A subordinate aside or takeaway line (`<em>`, `<p class="muted">`) stays at **body size**, rendered gray. `.muted` is a *color-only* class (canonical in `reference/deck.css`); never give it a reduced `font-size`, and don't reach for `.small`/`.tiny` to tone a line down. A line not worth body size belongs in `<deck>-note.html`.
- Component-internal text (pill labels, token chips, code blocks) keeps its native compact size.
- Cramped slide → **cut content or split the slide**. There is no slide budget.
...
**Phrases, not sentences.** Telegraphic noun phrases. Drop narrative connectors ("this means…", "in other words…", "essentially", "actually") and soft qualifiers ("very", "quite", "fairly"). Speaker narrates; the slide is a visual anchor. Applies to `h2` titles — short and abstract (3–6 words), not action-title sentences.

**Math is not prose.** A theorem statement, definition, or equation in a `.math-block` doesn't count toward the 40-word ceiling.

**One claim per line.** Multiple distinct claims joined by periods in one paragraph, `<li>`, `.highlight`, or `.card` → split each into its own `<p>` (or a `<ul>` for 3+ short claims). Applies inside `.highlight` / `.card` too: separate `<p>` siblings, not concatenated sentences.

**Math-comma-math — anti-pattern.** Starting a clause with math is fine; the failure is when the *previous* clause also *ended* in math: "For large $N$, $N\sigma^2$ dominates" reads across the comma as one expression ($N, N\sigma^2$). Insert a noun in the second clause ("the second term $N\sigma^2$ dominates") or restructure the boundary.

**Em-dash mid-sentence — anti-pattern.** "X is a sparsifier — pruning emerges" wraps awkwardly; the dash orphans. Replace with a colon plus `<br>` (one deliberate internal break, step 2 below) or two `<p>` tags. Em-dashes are only safe in technical glyphs (`7–8B`, `fill-in-the-middle`), `.cite` lines, and `h2` titles like "Proof — MGF bound".

**Numerical anchors next to closed-form formulas.** Pin a concrete running-example number next to any closed-form `R(D)`, `D(R)`, or quantity-as-function: `$R(D) = 1 - H_b(D)$, &nbsp;$R(\tfrac14) \approx 0.189$ bits`. Audience reads the formula; speaker says the number.

**When prose visibly wraps, fix in this order:**

1. **Compress.** Most overflow is wordiness. Collapse to noun phrases.
2. **Split** at a natural sentence/colon/clause boundary into two adjacent `<p>` tags. `<br>` only at a deliberate internal clause break (rare).
...
When something overflows: compress (Priority 1) → split the slide → move secondary detail to `<deck>-note.html`. **Never** shrink type. **Never** compress vertical rhythm (`margin`, `padding`, `line-height`) on prose. For multi-slide proofs, a "(continued)" `h2` plus a one-line recap is the canonical recovery — see Math-heavy → Proof.

**Visual element budget.** A content slide gets `h2` + `divider` + at most **5 child elements**. A `.math-block` counts as 1; a `<table>` counts by row count (header + N data rows); a `.highlight` counts as 1 regardless of internal `<p>` count. **`.math-block` + `<table>` + `.highlight` on one slide is already over budget — split before previewing.** This rule predicts overflow without rendering; honor it at draft time.

**Vertical budget (px arithmetic).** When the element budget is ambiguous, sum rule-of-thumb rendered heights (approximate; canonical numbers are what the browser renders):
...
Body budget: 720 px − slide padding (56 top + 48 bottom) − header block − bottom reserve ≈ **430–470 px**. Worked example: 3-line `aligned` block (~200 px) + 1-line `math-block` (~80 px) + two prose lines (~80 px) + `.highlight` (~70 px) ≈ 430 px — at the edge; hence the ceiling *at most one 3-line `aligned` block + one one-line `math-block` per slide*. Beyond that sum: split.
...
- Don't wrap 2–3-line content in `.cols + .card` — `.cols { flex: 1; align-items: stretch }` stretches short cards into tall hollow boxes. Use `.grid-2` (no flex stretch) with bare `<h3>` + `<p>` for short dichotomies.
- If a slide is too sparse to hold itself, merge or expand. Never pad.

### Style rules (informed by priorities)

1. **One idea per slide.** And **one exhibit** — one chart, table, diagram, theorem, proof chunk, or equation block. If two seem needed: one comparison (combine) or two points (two slides)?
2. **Speaker narrates; slide is a visual anchor.** Telegraphic phrases over sentences.
3. **Prose emphasis.** `**strong**` → Yonsei Blue. `*em*` → muted gray (never italic — see GOTCHAS). A whole subordinate line → `<p class="muted">` — same gray, still body size (Priority 0).
...
7. **Ghost deck test.** Read only the `h2` titles in sequence — they should outline the lecture arc. If not, fix the outline before drafting bodies.
8. **Spell out acronyms on first appearance.** `SSCD (Self-Supervised Copy Detection)`, `RTA (Random Token Addition)`, `MV / RV / TV (Matching / Retrieval / Template Verbatim)`; bare acronym after. Canonical ones the audience knows (LLM, MIA, DP, KL, MSE) are exempt. Expansion goes in the slide body where the term first appears, not a glossary slide.
9. **Visual-first — aim for a visual on every slide (default).** Prefer a diagram, figure, chart, schematic, or worked-math block over a text-only slide. Ask "what's the picture?" before "what are the bullets?". Full policy: **Visual richness**. Doesn't override rule 1 — *a* visual, not a collage.
10. **Self-contained slides (independent modules).** In a multi-lecture course, each slide stands on its own. No "next week", "previously", "(Wk N)", "see Lecture 3", "as we saw". A week index belongs only on a syllabus / course-map slide.

---

## Verifying without the toolchain (no-render fallback)

Canonical loop: headless Chrome → PDF → `pdftoppm` → PNG → visual read (`/audit-and-edit-deck`), plus `python3 scripts/lint-deck.py` / `find-wordy.py` / `outline-lint.py`. **Always prefer that pipeline when it works.** Confirm absence first (`command -v google-chrome pdftoppm python3 mutool convert gs`); use this fallback only when nothing is available or installable.

1. **Slide map by grep, never memory.** `grep -n 'class="slide' <deck>.html` lists every `.slide` div in document order. "Page N" = the Nth match, counting from 1 at the title slide (Conventions → Page numbering). Rebuild the map after every insertion/removal — never trust remembered line numbers.
2. **Overflow by arithmetic, not rendering.** Apply the Priority 2 vertical-budget table to every slide you touch; anything past ~430–470 px must be split *now*. The element budget (≤5 children; `.math-block` + `<table>` + `.highlight` = over budget) is the fast pre-filter.
...
5. **Flag the caveat.** This fallback cannot catch visual overlap, squashed math spacing (`\!` pulling glyphs together), drifted SVG overlays, or rendering surprises. Say so explicitly and recommend a real screenshot audit (`/audit-and-edit-deck`) once tooling is available.
...
3. **Sections** — `.section-slide.left` numbered divider, then that section's content slides. The centered `.bg-accent` variant only for dramatic interludes / statement slides, not structural breaks.
...
Before drafting, open the genre's exemplar and match its rhythm — section length, exhibit density, build-up pacing. Budget each planned slide against the Vertical budget (Priority 2) at draft time.
...
Never recolor for decoration. Color carries semantic load only when it's stable.
...
Rigorous statement. Assumptions, claim, citation. `.card` with an inline label (`<h3>Theorem N (Author, Year).</h3>`), assertion in a `.math-block`.
...
**Continuation slides.** The lead line names both the continuation *and* the move that continues:

```
Proof (continued) — <short paraphrase of the continuing step>
```

e.g. `<h2>Proof (continued) — Divide by the marginal</h2>`. Below the `h2`, one recap line of where the chain stands, then the new math. Never a bare `Proof (continued)` heading — the paraphrase lets a reader landing mid-proof reorient.
...
1. **Intuition** (1 slide) — picture, geometry, why to expect the result.
...
Auto-cycling animation is banned for talks (speaker loses pacing — see GOTCHAS). Use **progressive slides**: duplicate the slide N times; on slide $k$, color the first $k$ steps Yonsei Blue (active/established) and the rest `--gray-text` (upcoming). Speaker advances at their own pace.
...
Two related equations stacked vertically → single `<div class="math-block">` with `\begin{aligned}`, **not** two adjacent `math-block` divs — adjacent blocks add too much vertical gap and read as separate ideas.
...
**Exception**: when the second equation is a *key conclusion* deserving its own moment (final result, closing claim), promote it to its own `math-block` — the visual gap then becomes signal.

**Tight-margin recipe (scoped margin reduction).** Each `.math-block` carries ~30 px of vertical padding/margin; two adjacent blocks ≈ 60 px of dead space — often what pushes a closing `.highlight` or the footer clearance over the edge. When a slide is tight *specifically because of math-block chrome* (typical on dense proof slides), reduce margins at the narrowest scope:

1. **One slide, one or two blocks:** inline `style="margin: 8px 0;"` on each affected `.math-block`.
2. **Several flagged slides in one deck:** a scoped per-deck class, tagging only those slides:
...
Never touch the global `.math-block` rule in `reference/deck.css`, and never apply the tight class deck-wide — it's a targeted rescue for proof-math slides, not a new baseline. Still over budget after tightening → split (Priority 2).

### Multi-step proof pattern

For derivations spanning 4+ logical steps, bracket the sequence:

1. **Setup** — define variables and the problem (one slide).
2. **Outline** — the *target* (e.g., the Bayes formula about to be simplified) in a `math-block`, then an `<ol>` of step labels. Previews the path.
3. **Step 1 … Step k** — one logical step per slide, body math at body size. Continuations use the `Proof (continued) — <paraphrase>` template above.
4. **Recap** — a single `aligned` block showing the unified equation chain, each step labeled above its relation symbol via `\stackrel{(k)}{=}` or `\stackrel{(k)}{\approx}`. Don't reuse the bulleted outline as a recap — outline previews step *labels*, recap shows the equation *chain*.
...
**Recap label choice — descriptive over numeric.** Numeric `\stackrel{(k)}{=}` cross-references step slides — useful when steps were big and named. When the *algebraic move* is itself non-obvious (e.g., "split $g^2/2 = g^2 - g^2/2$", "use $\partial p = (\partial \log p)\,p$", "factor $-\partial_x$"), label the move directly so the recap is self-contained:
...
Use `\text{...}` for upright Roman; keep each label under ~25 characters so it doesn't widen the relation column. Mix-and-match is fine — numeric where a step needs a callback, descriptive where the move is the explanation.

### Recipe-first derivation

When introducing a parameterized formula whose specific shape isn't obvious (DDIM's $\mu_n = \sqrt{\bar\alpha_n}\,X^{(0)} + \sqrt{1-\bar\alpha_n-\sigma^2}\,\epsilon_{n+1}$, score-network reparameterizations, …), don't open with the formula and verify it. Open with a **recipe** carrying named unknowns, **derive the unknowns** from the desired property, then **read off** the parameterization.

Three-slide arc:

1. **Recipe.** The construction with placeholders for unknown coefficients. `\underbrace` labels each ingredient (signal / recycled / fresh, or whatever the decomposition is). Identify what you'll determine.
2. **Constraint.** Impose the property you want (variance budget, marginal match, normalization). One equation per unknown ⇒ solve. Surface the *free* parameters explicitly.
3. **Read-off.** Substitute the solved coefficients back; convert to the standard form (e.g., the conditional density).

Every coefficient came from a constraint, none was hand-picked — reverse order teaches the algebra but obscures the design intent. Don't apply when the formula has a clean independent motivation (Bayes' rule, KL, an established theorem).

### Substitution

When applying an abstract result by relabeling variables, show the abstract form first, then the substitution arrow, then the concrete form — never jump straight to the substituted form.
...
Use `\underbrace{...}_{\text{name}}` to label parts of a long equation in place. **Label the per-term operand, not the whole sum** — name the per-step object the reader will reason about:
...
When deriving B from A, state A first as a self-contained fact (own paragraph + `math-block`), *then* derive B. Don't tuck A into a trailing parenthetical or "where …" clause — that hides the prerequisite under the consequence.
...
The on-screen `slide-num` indicator (and any user reference to "page N") counts **every** `<div class="slide">` in document order — title, TOC, section dividers, content, recap, end-slide. `deck.js` enumerates them with `querySelectorAll('.slide')` and shows `(current+1) / total`. "Page 23" = count from 1 at the title.

When editing, match slides by content (`<h2>` text, distinctive class), not position — numbers shift on any insert/remove. Without the render toolchain, build the page↔slide map with `grep -n 'class="slide' <deck>.html` (Verifying without the toolchain).

**One indicator only — the bold `.slide-num`** (`deck.js` injects/updates it; `position: fixed; bottom-right; font-weight: 700`). Do **not** add a per-deck `<script>` injecting a second `.page-num` element — two indicators render as duplicate page numbers. If a deck has both, delete the `.page-num` injector script and its `.page-num` / `.title-slide .page-num` CSS; keep the bold `.slide-num`. (Hidden in print/PDF by `@media print` — expected.)

### Citations

`<div class="cite">…</div>` format: `Authors (in venue order), "Title", Venue YYYY` — no arXiv ID unless explicitly requested, never arXiv-only attribution (e.g. `Isik et al., arXiv:2102.08329.` is wrong). Use the **venue's** author order (PMLR / NeurIPS / JMLR / journal proceedings page), not arXiv's — they sometimes differ. `et al.` is acceptable for 4+ authors after first reference; spell out all authors on the first slide that introduces a paper.
...
**One dedicated line per citation.** Each citation renders on a single line (`.cite` is centered, capped at 60% width). Two papers → two lines: joined inside one `.cite` with `<br>`, or two adjacent `.cite` blocks. Never let a citation wrap mid-title.
...
- **Abstract-bullet slide** — short form: `Author et al., NameOrAcronym, Venue YYYY.` (e.g., `Rafailov et al., DPO, NeurIPS 2023.`). Use when the full title would wrap.

**Position rule.** The citation lives at the **bottom** of the slide. Constraints, in priority order:
...
- `class="cite cite-left"` — left-aligned, full-width (capped at 92%), `bottom: 48px` so it clears the brand footer. For a long citation when the slide isn't dense on the right.
- `class="cite cite-right"` — right-aligned, full-width (capped at 92%), `bottom: 18px`. Only when the lower-left region must stay clear (rare).

Both classes live in the deck's local `<style>`; copy the definitions from any deck that already uses them. Default centered cite is still preferred.

**Figure citations.** For a captured paper figure, cite the figure number: `Author et al., Venue YYYY — Figure N.` (Visual richness → Figure-capture protocol.)
...
```
Tokens: `.kw` (keyword, blue, 700), `.fn` (function name, accent, 600), `.cm` (comment, gray), `.str` (string, green, 600). Renders at 1.25rem / weight 500. Pseudocode is fine; full Python listings rarely fit (move to the note file). Note the escaped `&lt;` — literal `<` in content is an HTML hazard (see GOTCHAS for the math case).
...
```
Notes:
- **`align-items: start`** — `center` floats short bullet lists midway down a tall figure (reads as a void on top).
- **Image height ceiling by layout**:
  - **Side column with bullets** (figure left, ≤4 short bullets right): `max-height: 380–430px`.
  - **Stacked (bullets below image)** with **1–3 short bullets**: `max-height: 380px`.
  - **Stacked with 4+ bullets** or a `.highlight` below: `max-height: 320–340px` — past that, bullets push into the `.cite` and brand footer.
  - **Single-figure slide** (only `h2 + divider + img + cite`): `max-height: 470px`.
  - When `max-height` is at the lower end and the image still feels small, prefer the **Image-first / description-follows** pattern below rather than removing bullets.
- **`<br>` inside `<li>`** is allowed for wrap-control next to a narrow column (Priority 1 step 2: one deliberate internal break).
- Figure stays in the talk folder (`<talk>/figs/<file>.png`); `bundle.py` inlines local `<img>` references as data URIs.

**Image-first / description-follows.** When a single image+bullets slide overflows — image cramped, bullets clipping the brand footer, or both — split into two consecutive slides: a full-bleed image slide with at most one orienting sentence, then a "Reading the plot" slide carrying the bullets. The image breathes at `max-height: 460–500px`; the analysis stays at body size. Use for information-dense figures (multi-panel plots, eigenvalue distributions, qualitative comparison grids).
...
Don't apply to simple figures (single plot, simple schematic) — one slide handles it. Reach for the split *after* the image-height ceilings above can't fit.
...
Use a counter-based `::before` for step numbers — default `<ol>` markers are too small at slide font sizes. Pad the step body left so the number visibly precedes the text:
...
**Build-up no-fade.** When several consecutive slides differ only by a single value (Lloyd–Max iteration, Bayes-step coloring, table-cell update), the engine's per-element fadeIn (`animation: fadeIn 0.4s ease both` with 0.06–0.30s stagger) makes navigation flash on every click. Disable it on those slides only, with a per-deck class:
...
Keep the fade-in on every other slide. Complements (does not replace) the multi-slide proof build-up — see Math-heavy → Build-up.

**Recall card.** When a slide depends on a definition the audience saw 5+ slides ago (typical-set definition, AEP, KKT, a specific lemma), lead with a small recall card:
...
Use only when the dependency is non-obvious; one per slide max. (Note the `&lt;` — literal `<` inside math garbles rendering; see GOTCHAS.)
...
1. Does the SVG carry a `style="max-width: <small>"` constraint? Remove it (`max-width` belongs on the wrapper `<div>`, not the SVG).
2. Is the layout `.cols` (two equal-width children)? Switch to `.cols` with `.col-1-3` (text) + `.col-2-3` (diagram).
3. Is the SVG viewBox unnecessarily small (e.g. `220×130`)? Bump to `~400×240` so labels render at readable absolute pixel sizes — labels at `1.0–1.4rem` need a viewBox where `font-size` math lands in a sensible scale.
...
For pure single-diagram slides, drop the cols and center the SVG with `max-width: ~960px; margin: 24px auto`.
...
If the slide is already at element budget and the exercise would push past the footer, move it to the *sibling slide that introduces the fact the exercise verifies* — not the slide that uses the fact.
...
For boxed nodes (more visual weight), use `.diagram-flow` + `.diagram-box` — at **full slide width only**, never inside `.cols`/`.grid-*` (see GOTCHAS). For a plain inline chain, the bare flex above is lighter and fits more nodes.

---

## Visual richness — figures, diagrams & TODO-marks

**Policy: target visually rich slides.** Every content slide should aim to carry an *exhibit* — diagram, schematic, chart, figure, table, or worked-math block — with text reduced to a short framing line. Bare bullets are a fallback, never the goal.

**Three sources of visuals, in priority order:**

1. **Real paper / public figures — preferred whenever a citable source exists.** If a paper or well-known blog post already has the figure that makes the point (panda→gibbon, the CLIP Figure 1 similarity grid, a COMPAS bar chart, a BadNets trigger), **capture and reuse it with an explicit citation** rather than redrawing — more credible, faster to produce correctly. Follow the **Figure-capture protocol** below. Reuse figures already vetted in sibling decks (e.g. `courses/privacy/lectures/03-memorization/figs/`) rather than re-fetching.
2. **Inline HTML + SVG concept diagrams** — for ideas with no single canonical source figure (a mechanism in your own words, a cross-paper comparison, a made-up worked example). Build structure in HTML/SVG (boxes, circles, arrows, overlapping bell curves, 2×2 grids, flows, pipelines, trees). Plain-text labels in SVG `<text>` — never `$…$` there (KaTeX skips SVG; see GOTCHAS). One semantic color per role (Yonsei blue = focus, gray = context, `--warn` red = the bad case). Shipped examples to imitate: overlapping-distribution "indistinguishability" curves, knowledge×timing 2×2 map, layered trust stack, randomized-response coin tree, Laplace-noise bell, federated-learning fan-in, linkage Venn (`courses/trustworthy-ai/lec01-introduction.html`, `lec02-privacy-dp.html`).
3. **TODO-marks** — when a real figure or richer diagram can't be produced now (no network, fiddly crop, needs design time), **do not ship a bare slide**. Leave a marker where the visual belongs:

   ```html
   <!-- TODO real figure: <what to show>, <source paper / Fig N> -->
   ```

   First-class authoring debt, not silent omissions. `grep -rn "TODO real figure"` finds every slide still missing its visual.

**When a well-known figure for the idea exists, capture it — don't redraw it.** If you catch yourself building an original HTML+SVG diagram for a concept a specific paper or blog post already illustrates (e.g. the CLIP paper's Figure 1 image×caption grid), stop and capture the real figure (source 1) with citation per the **Figure-capture protocol**. Fall back to an original concept diagram (source 2) only when no clean source figure exists, or the panel is unsuitable as-is (e.g. it reproduces extracted private training data — see "Prefer methodology figures" below).

**Make diagrams big.** Concept SVGs are routinely drawn too small. Full-width single-diagram slides: wrapper `max-width: 820–920px`, SVG `<text>` `font-size: 15–20` (viewBox units), bold for primary labels. Diagrams paired with bullets in a grid column: `max-width: 360–440px`, `font-size: 13–16`. Bump **both** the wrapper width and the font-size — a wider wrapper alone leaves labels proportionally small (GOTCHAS → "Concept SVG renders too small"). See Recipes → Diagram dominates.

**Still one exhibit per slide.** Two visuals ⇒ two slides (Style rule 1). Empty space *below* a centered diagram is fine; middle voids are not (Priority 3).

### Figure-capture protocol (third-party papers)

When pulling a real figure from an arXiv/venue paper into a deck:

- **Crop the main figure caption out.** The "Figure N: …" line is for the paper reader. Keep subcaption labels `(a)`, `(b)` only when the speaker references them.
- **Cite the source figure number** in `.cite`: `Author et al., Venue YYYY — Figure N.`
- **Prefer methodology figures** (plots, schematics, algorithm diagrams) over panels that reproduce extracted training images or other third-party copyrighted content. For those panels, describe the example in text with strong attribution instead.
- **Crop tightly, with headroom.** Drop page headers ("Published as a conference paper at…"), running titles, and adjacent figure/table content — but leave a few px of headroom on every side (tabs and axis titles sit a few px outside the apparent bounding box; see GOTCHAS). Typical flow: `pdftoppm -r 200 paper.pdf prefix`, then crop with `sips --cropToHeightWidth H W --cropOffset Y X` or PIL — eyeball the offsets, iterate twice, then **Read the resulting `figs/*.png` directly** to confirm (never trust a possibly-cached in-slide render).
- **Render at ≥180 DPI** when extracting via `pdftoppm`; lower DPI pixelates on a projector.
- **Store under `<talk>/figs/`.** `bundle.py` inlines `.png`/`.jpg`/`.svg`/`.webp`/`.gif` from any path relative to the deck.
- **Downsample to ~1200 px max width before bundling** (`sips --resampleWidth 1200 figs/big.png --out figs/big.png`). Full-res captures (2000–3500 px) base64-bloat the standalone bundle past 50 MB (see GOTCHAS).

---

## Companion note files

Detail that doesn't fit on the slide (full-sentence explanations, derivations the proof slide skipped, secondary examples, "FYI" context) goes in `<deck>/<deck>-note.html` — one section per slide, in slide order, headed by the slide's title. Plain HTML so KaTeX `$…$` / `$$…$$` works as in the deck. No `.deck` / `.slide` engine; a simple `<article>` per slide is enough.

For lectures, the note file is also the home for the **expanded proof** when the slide carries the abbreviated version.

**Migration check (trim → note).** Whenever you trim detail off a slide on the grounds that "it belongs in the note file", open the paired `-note.html` and verify the content is actually there; if not, migrate it *in the same edit*. Deleting on the assumption it was already covered silently destroys content.

**Speaker-script variant.** One `<article>` per slide in deck order, `<h2>` = the slide's title, then 1–2 sentences of what to actually say, then a `Key takeaway:` line (a blue `.kt` span). Doubles as the deck's outline-of-record. Keep each entry ≤ ~50 words of script. Not a paper draft, not a transcript.

## Technical supplement decks (concept-first split)

For a course pitched below the math's native level, split each lecture: the **main deck** stays concept-first — motivation, pictures, at most **one glanceable formula per concept** — and a sibling **`<deck>tech.html`** holds the formal version (multi-line derivations, algorithm boxes, definition cards with quantifiers, test statistics). The supplement is optional. Established across the 15-deck `courses/trustworthy-ai/` course (`lecNNtech.html`).

Rules that keep the split clean:

- **Stay/move rule.** *Stays* in the main deck: one short single-expression formula readable at a glance (`‖δ‖ ≤ ε`, a one-line rate equality). *Moves* to the supplement: anything multi-line; algorithm boxes with math; formal-definition cards with quantifiers; Hessians, sums-over-subsets, expectations-with-penalty, likelihood ratios, z-statistics.
- **No back-references.** When you move a formula out, leave a one-line plain-English statement so the main slide stands alone (Style rule 10). Do **not** point the main slide at the supplement — both decks are self-contained; the supplement is discoverable via `OUTLINE.md`, not cross-refs.
...
Every folder carries an `OUTLINE.md`: root navigator, folder overview, leaf subfolder. Leaf files list every deck's section table with `file:line` pointers and every named theorem/lemma/key formula with its line number — the canonical map for finding "where does proof X live" without grepping.

### Read-side rule — consult before writing

**Before writing or substantively rewriting any slide content, read the relevant `OUTLINE.md` files** — seconds of cost, avoids redefinition, contradiction, duplication. Two triggers:

1. **In-track continuity (same lecture series).** When extending a multi-deck series — e.g., writing `privacy/lectures/04-mia/mia3-theory.html` — open `privacy/lectures/04-mia/OUTLINE.md` and `privacy/OUTLINE.md` first. Confirm what notation, definitions, and theorems earlier decks established. Don't redefine $(\varepsilon, \delta)$-DP if `privacy/lectures/01-dp/dp4-approximate-dp.html` (or `dp8-fl.html`) covered it — refer back via a brief "Recall (Lecture X)". Uncovered prerequisite → decide: (a) one-slide recap, (b) point students to the prior deck, or (c) defer the topic.

2. **Cross-folder reuse (same topic, different track).** When the topic may already live elsewhere — diffusion (`infotheory/lectures/07-diffusion/` vs `privacy/lectures/02-generative/` vs `dllm` talks), DP (`privacy/lectures/01-dp/` vs `privacy/lectures/04-mia/mia1-foundations.html`), MI bounds (`infotheory/lectures/05-mi/` vs anywhere CLIP/InfoNCE comes up) — open the **root `OUTLINE.md` quick-lookup table** and the relevant leaf files in the *other* folder. Decide explicitly: reuse as-is, adapt, link via "see also", or deliberately contradict (with rationale). Don't rederive a theorem in track B that has a clean statement and proof in track A — link to `<file>:<line>` instead.

When an OUTLINE entry is too coarse, descend into the cited `<file>:<line>` and confirm. The OUTLINE points; it does not replace reading the deck.

### Write-side rule — keep outlines accurate

**Maintenance rule (non-negotiable):** any slide edit that changes a section boundary, line range, or named theorem location requires the corresponding `OUTLINE.md` line numbers to be updated *in the same change*. Adding a slide → add the entry. Removing → remove it. Renaming a section → rename it everywhere cited (leaf, folder, root quick-lookup). Bulk renumbering after a multi-slide insertion is part of the edit, not a follow-up.

If you cannot locate a topic from the OUTLINE files, they are stale — fix them, don't work around them. `python3 scripts/outline-lint.py` verifies every cited `file:line` (file exists, line within range) — run it after any edit that shifts line numbers; it cannot check the line still holds the *claimed* content, so spot-check after big restructures. Without `python3`: `sed -n '<N>p' <file>` (Verifying without the toolchain).

When creating a new deck, add a stub entry in the leaf `OUTLINE.md` before writing the deck (file path + topic line). Cross-references in the root quick-lookup table get added when the content is actually present.
...
1. Styled variation of an existing component? Use inline `style=` (with `var(--…)`, never hardcoded colors — except SVG presentation attributes, which can't take `var()`; see GOTCHAS).
...
New components must document: name, purpose, default + `.bg-accent` appearance, an example, token dependencies. Candidates worth adding when reuse creates friction: `.theorem`, `.proof-step`, `.intuition-callout`. Until then, the recipes above using existing `.card` / `.math-block` / `.highlight` are the canonical form.
